...
func newTestSeq() *seqConversationCacheRedis {
	mgocli, err := mongo.Connect(context.Background(), options.Client().ApplyURI("mongodb://owlIM:owlIM123@172.16.8.48:37017/owlim_v1?maxPoolSize=100").SetConnectTimeout(5*time.Second))
	if err != nil {
		panic(err)
	}
	model, err := mgo.NewSeqConversationMongo(mgocli.Database("owlim_v1"))
	if err != nil {
		panic(err)
	}
	opt := &redis.Options{
