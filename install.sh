...
# Remember, it should be a combination of 8 or more numbers and letters. If you want to set a different password, uncomment and replace "owlIM123".
# PASSWORD=owlIM123
...
function set_openim_env() {
    ...
    # If the PASSWORD environment variable is not set, it defaults to 'owlIM123'
    # This password applies to redis, mysql, mongo, as well as accessSecret in config/config.yaml
    if [ -z "$PASSWORD" ]; then
        PASSWORD="owlIM123"
        debug "PASSWORD is not set. Defaulting to 'owlIM123'."
    fi
...
function cmd_help() {
    ...
    color_echo ${BLUE_PREFIX} "-p,  --password      ${CYAN_PREFIX}set password (default: owlIM123)${COLOR_SUFFIX}"
